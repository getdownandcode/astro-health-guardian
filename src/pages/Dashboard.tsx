...
const Dashboard = () => {
  ...
  const filteredAstronauts = user?.role === "astronaut" && user?.astronautId
    ? astronauts.filter(a => a.astronaut_profile.id === user.astronautId)
    : astronauts;
    
  // Risk level counts
  const riskCounts = {
    Critical: filteredAstronauts.filter(a => a.prediction.risk_level === "Critical").length,
    High: filteredAstronauts.filter(a => a.prediction.risk_level === "High").length,
    Medium: filteredAstronauts.filter(a => a.prediction.risk_level === "Medium").length,
    Low: filteredAstronauts.filter(a => a.prediction.risk_level === "Low").length,
  };
  ...
  return (
    <Layout>
      <div className="space-y-6">
        ...
        {/* Astronaut List */}
        <div className="space-y-3">
          <h2 className="text-xl font-medium">Astronaut Health Status</h2>
